...
const shared = [
  ...
  `The file is read-write: after any live gather that fills a gap or goes deeper than what was there, write it back via mergeGithubEvidence/mergeLogsEvidence (lib/evidence-file.mjs) before finishing this test, so a sibling dispatched after you — or another cluster sharing the same repo/workload — reads the enriched entry instead of re-fetching it.`,
