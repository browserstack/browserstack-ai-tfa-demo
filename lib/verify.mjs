...
function runOne({ row, field, value, scope, runProbe, envVar, env, candidates, route = "cli" }) {
  ...
  if (template) {
    ...
    const { text: toolName, missing } = fillPlaceholders(String(row.mcpProbe.tool ?? ""), merged);
    if (missing.length > 0) {
      return fail("unknown", {
        nextAction: `The MCP tool name for this capability is unresolved (${missing.join(", ")}) — answer it during setup, then re-run.`,
      });
    }
