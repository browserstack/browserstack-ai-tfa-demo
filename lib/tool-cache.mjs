...
export function isRunnable(command) {
  ...
  if (op) {
    return {
      ok: false,
      reason: `'${op}' is a shell operator — run it OUTSIDE the wrapper (e.g. \`cached-exec … 'gh api X' | jq .y\`) so one cached fetch can serve several different filters. Metacharacters inside a quoted argument are fine.`,
    };
  }
