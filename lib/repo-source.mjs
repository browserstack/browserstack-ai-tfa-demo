...
function git(dir, args) {
  return execFileSync("git", ["-C", dir, ...args], {
    ...
    // A fetch can reach the network, and a credential or host-key prompt reads
    // from /dev/tty — which `stdio: ignore` does not close off. Without these a
    // single unattended repo could hang the whole batch indefinitely.
    timeout: 60_000,
...
/** True when a ref name is safe to hand to git as an argument.
 *
 * Not a style rule. `git fetch origin <ref>` parses options in that position, so a
 * value beginning with `-` is read as one: `--upload-pack=<cmd>` runs <cmd>
 * locally, demonstrated against a clone with a local-path origin. And any refspec
 * is accepted there, so `+refs/heads/*:refs/heads/*` would rewrite local branches,
 * contradicting this module's own guarantee that a fetch "never moves a branch or
 * the working tree".
 *
 * The value arrives from RCA_SHIPPING_BRANCH — from the gate and a connector
 * skill, the same class of externally-supplied input discovery.mjs filters field
 * by field. execFile gives no protection, because the injection is an argv
 * POSITION rather than a shell metacharacter. */
function isSafeRef(ref) {
  const r = String(ref ?? "");
  return r.length > 0 && r.length <= 255 && /^[A-Za-z0-9][A-Za-z0-9._\/-]*$/.test(r) && !r.includes("..");
}
...
export function ensureCommit(dir, sha, branch) {
  ...
  try {
    ...
    // Fully qualified, so it can never be read as an option or a wildcard refspec.
    git(dir, ["fetch", "--quiet", "origin", branch ? `refs/heads/${branch}` : sha]);
  } catch {
    return false;
  }
