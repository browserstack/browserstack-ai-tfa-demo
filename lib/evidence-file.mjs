...
// artifact instead of re-running the same forge, runtime and log-store calls.
//
// Distinct from the tool cache: that one is an
...
/**
 * Bring a document written before the section rename forward.
 *
 * These artifacts are per-build and survive a resume, so a run that started before
 * the rename lands here afterwards. Read silently, the old shape produced the worst
 * possible outcome rather than an error: `doc.code` was undefined, so
 * `hasTrustworthyPrList` returned false and `deployShas` returned nothing, and
 * `recomputeCoverage` reported the repo GAPPED while its evidence sat in the file —
 * which tells a coordinator to report "no culprit PR identified" for a repo whose
 * PR window had already been fetched. Confidently wrong, from a successful read.
 *
 * Migrating on read rather than refusing, because refusing would strand an
 * in-flight run for a rename that loses no information. One-way and idempotent.
 */
function migrate(doc) {
  if (doc && doc.github && !doc.code) {
    doc.code = doc.github;
    delete doc.github;
  }
  for (const entry of Object.values(doc?.logs ?? {})) {
    if (!entry || Array.isArray(entry.sweeps)) continue;
    // The two product-named slots became one list keyed by the source that
    // produced it. `via` is what the new shape needs and the old shape only
    // implied, so it comes from the field name it was stored under.
    const sweeps = [];
    for (const [via, leaf] of [["kubectl", entry.kubectlSweep], ["victorialogs", entry.victorialogs]]) {
      if (leaf) sweeps.push({ via, ...leaf });
    }
    entry.sweeps = sweeps;
    delete entry.kubectlSweep;
    delete entry.victorialogs;
  }
  return doc;
}
...
export function readBaseFile(filePath) {
  ...
  try {
    return migrate(stripMarkers(JSON.parse(readFileSync(filePath, "utf8"))));
  } catch {
...
function foldCode(target, repo, entry) {
...
/**
 * Merge log sweeps by the source that produced them.
 *
 * This entry used to carry exactly two slots, `kubectlSweep` and `victorialogs` —
 * two product names baked into a persisted SCHEMA. A team on any third log source
 * had nowhere to put its evidence, and a team on neither had two permanently null
 * fields; `bin/evidence-show.mjs` printed both names to every customer. Keyed by
 * `via` instead: one entry per source actually used, however many that is.
 */
function mergeSweeps(cur = [], incoming = []) {
  const by = new Map();
  for (const s of [...(cur ?? []), ...(incoming ?? [])]) {
    if (!s) continue;
    // A via-less sweep is LABELLED, never dropped. Dropping it deleted evidence
    // already persisted in the base file the moment any coordinator contributed
    // anything unrelated — leaving `{sweeps: [], gap: null}`, which isCovered
    // reads as COVERED, so recomputeCoverage reported the workload covered with
    // zero evidence and a coordinator skipped the live sweep. An ugly label is
    // strictly better than a silent deletion that reads as success.
    const via = String(s.via ?? "").trim() || "unattributed";
    // Keyed case-insensitively: `via` is freeform agent text, so `victorialogs`
    // and `VictoriaLogs` were two entries for one source — "the later shard wins"
    // stopped holding and the stale block survived beside the fresh one.
    const key = via.toLowerCase();
    const prev = by.get(key);
    by.set(key, prev ? { ...prev, ...s, via: prev.via, block: pickLeaf(prev.block, s.block) } : { ...s, via });
  }
  // Bounded. `via` is freeform agent text and this document is read IN FULL by
  // every coordinator, so twelve writers each spelling a source differently used to
  // mean twelve entries where the old two-slot schema meant one. The cap is high
  // enough that no real stack reaches it and low enough that a naming mishap
  // cannot bloat the file every coordinator loads.
  const merged = [...by.values()];
  return merged.length > MAX_SWEEPS ? merged.slice(0, MAX_SWEEPS) : merged;
}

const MAX_SWEEPS = 16;

const hasContent = (sweeps = []) => sweeps.some((s) => s?.block);

function foldLogs(target, workload, entry) {
  const cur = target[workload] ?? { clusterIds: [], sweeps: [], gap: null };
  const next = {
    clusterIds: [...new Set([...(cur.clusterIds ?? []), ...(entry.clusterIds ?? [])])],
    sweeps: mergeSweeps(cur.sweeps, entry.sweeps),
    gap: cur.gap ?? null,
  };
  if (entry.gap === null || entry.gap === undefined) {
    if (hasContent(entry.sweeps)) next.gap = null;
  } else if (!hasContent(next.sweeps)) {
    next.gap = entry.gap;
  }
...
export function readEvidenceFile(filePath) {
  ...
  for (const shard of readContribs(filePath)) {
    const m = migrate(shard);
    for (const [repo, entry] of Object.entries(m.code ?? {})) foldCode(doc.code, repo, entry);
    for (const [wl, entry] of Object.entries(m.logs ?? {})) foldLogs(doc.logs, wl, entry);
 ...
 * `baseline` is `{ref, isFallback}` — the last-green ref, or the fallback when
 * there is none
...
/** Read-modify-write merge into `doc.code[repo]`. `entry` shape:
 ...
 * `{ clusterIds, sweeps: [{via, block, gap}], gap }` — one sweep per log source
 * actually used, each naming the tool or server it came from.
...
function loadOwnShard(basePath, writerId, nowMs) {
  ...
  if (!existsSync(p)) {
    return { path: p, doc: { writerId: String(writerId), generatedAtMs: nowMs, code: {}, logs: {} } };
  }
  try {
    // Migrated here too, not only in readBaseFile. This is the WRITE path: a
    // coordinator resuming onto its own pre-rename shard hit
    // `Cannot read properties of undefined` in contributeCodeEvidence, and
    // contributeLogsEvidence dropped the old sweep content permanently, because
    // migrate() then saw an already-migrated entry.
    return { path: p, doc: migrate(JSON.parse(readFileSync(p, "utf8"))) };
  } catch {
    return { path: p, doc: { writerId: String(writerId), generatedAtMs: nowMs, code: {}, logs: {} } };
  }
}
...
export function contributeCodeEvidence(basePath, writerId, repo, patch, nowMs) {
  ...
  const entry = doc.code[repo] ?? { deployState: null, prsInWindow: [], gap: null };
...
export function contributeLogsEvidence(basePath, writerId, workload, patch, nowMs) {
  const { path, doc } = loadOwnShard(basePath, writerId, nowMs);
  const entry = doc.logs[workload] ?? { clusterIds: [], sweeps: [], gap: null };
  if (patch.sweeps !== undefined) entry.sweeps = mergeSweeps(entry.sweeps, patch.sweeps);
...
//
// For a code entry there is a further trap, hit for real in testing: an
...
export function recomputeCoverage(filePath, requested, nowMs) {
  ...
  doc.coverage = {
    reposCovered: repos.filter((r) => isCovered(folded, "code", r)),
    reposGapped: repos.filter((r) => !isCovered(folded, "code", r)),
    ...
    reposWithUntrustedPrList: repos.filter(
      (r) => isCovered(folded, "code", r) && !hasTrustworthyPrList(folded, r),
    ),
  };
  doc.generatedAtMs = nowMs;
  writeEvidenceFile(filePath, doc);
...
export function deployShas(filePathOrDoc) {
  ...
  const pins = {};
  const source = {};
  for (const [repo, entry] of Object.entries(doc?.code ?? {})) {
