...
export function persistClusters(csvPath, csvState, assignment = {}) {
  ...
  // Object.hasOwn, not a plain lookup: `assignment` is agent-authored JSON, and a
  // row whose id happened to be `toString` resolved through the prototype chain to
  // a function, satisfying the every-row-assigned check with garbage.
  const assignedFor = (id) => (Object.hasOwn(assignment, id) ? assignment[id] : undefined);
  const missing = rows.filter((r) => !String(assignedFor(r.testRunId) ?? "").trim());
  if (missing.length > 0) {
    throw new Error(
  ...
  for (const row of rows) row.cluster_id = String(assignedFor(row.testRunId)).trim();
