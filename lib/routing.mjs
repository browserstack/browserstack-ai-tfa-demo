...
export function buildManifest(config, discovered = []) {
  ...
  for (const entry of Object.values(config?.evidenceRouting ?? {})) {
    ...
    // The entry is WIDER than {available, via} because the setup gate needs
    // per-target proof, a per-field tag and a gap classification that the
    // original two fields cannot carry. Widening is additive on purpose:
    // routeAsk() branches on `.available` alone and reads `.via`, so every
    // existing consumer is unaffected.
    manifest[cap] = found
      ? {
          available: true,
          via: found.via ?? null,
          resolvedScope: found.resolvedScope ?? null,
          unresolvedFields: found.unresolvedFields ?? [],
          tag: found.tag ?? "detected",
          gapClass: found.gapClass ?? null,
        }
      : {
          available: false,
          via: null,
          resolvedScope: null,
          unresolvedFields: [],
          // No tag yet: the interview may still answer or skip this capability,
          // and verification may still fail it. Discovery finding nothing is not
          // itself one of the gate digest's field states.
          tag: null,
          gapClass: null,
        };
  }
