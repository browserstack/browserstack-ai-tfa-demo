...
/** Loose two-way containment. An MCP fingerprint of "prometheus" should match a
 *  server advertised as "prometheus", "mcp__prometheus__execute_query" or
 *  "claude_ai_Prometheus" — the naming is host-specific and not worth a schema. */
...
function fileMatches(fingerprint, path) {
  ...
  const p = norm(path);
  if (!f || !p) return false;
  return p === f || p.startsWith(`${f}/`) || p.includes(`/${f}/`) || p.endsWith(`/${f}`) || p.startsWith(f);
}
...
function isFingerprintedAnywhere(table, kind, name) {
  const key = kind === "executables" ? "executables" : "mcp";
  for (const row of Object.values(table)) {
    const list = row?.fingerprints?.[key] ?? [];
    if (key === "executables" && list.some((e) => norm(e) === norm(name))) return true;
    if (key === "mcp" && list.some((f) => mcpMatches(f, name))) return true;
  }
  return false;
}
...
export function interpolate(template, scope = {}, { leaders = [] } = {}) {
  ...
  const missing = [];
  const command = t.replace(/\{([A-Za-z0-9_]+)\}/g, (_, field) => {
  ...
  return { ok: true, command, argv: r.argv };
}
