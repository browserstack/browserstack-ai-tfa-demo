 ...
 *  - always:       discovery resolves capability AND scope
...
export const RESOLVABLE = new Set(["always", "partial", "always-asked"]);
...
export function mergeOverlay(shipped = {}, overlay = null) {
  ...
  for (const [cap, row] of Object.entries(overlay)) {
    ...
    const clean = { ...row };
    for (const field of OVERLAY_FORBIDDEN) delete clean[field];
    table[cap] = { ...(table[cap] ?? {}), ...clean };
  }
  return { table, violations };
}

function validateMcpProbe(cap, mcpProbe, violations) {
...
export function validateTable(config, table = {}) {
  ...
  for (const [cap, row] of Object.entries(table)) {
    ...
    const needsProbe = row?.resolvable === "always" || row?.resolvable === "partial";
    ...
    if (hasMcpProbe) validateMcpProbe(cap, row.mcpProbe, violations);
