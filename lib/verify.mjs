...
import { interpolate } from "./discovery.mjs";
...
function nextActionFor(errorClass, ctx) {
  const build = NEXT_ACTIONS[errorClass] ?? NEXT_ACTIONS.unknown;
  const action = build(ctx);
  // The guarantee is unconditional, so it is enforced here rather than trusted.
  return action && action.trim() ? action : NEXT_ACTIONS.unknown(ctx);
}
...
function toolPresent(row, env) {
  const executables = row?.fingerprints?.executables ?? [];
  const mcp = row?.fingerprints?.mcp ?? [];
  const have = (env?.executables ?? []).map((e) => String(e).toLowerCase());
  const servers = (env?.mcpServers ?? []).map((s) => String(s).toLowerCase());
  if (executables.some((e) => have.includes(String(e).toLowerCase()))) return true;
  return mcp.some((f) => servers.some((s) => s.includes(String(f).toLowerCase())));
}
...
function templateFor(row, field) {
  const token = `{${field}}`;
  if (typeof row?.scopeProbe === "string" && row.scopeProbe.includes(token)) return row.scopeProbe;
  if (typeof row?.probe === "string" && row.probe.includes(token)) return row.probe;
  return typeof row?.probe === "string" ? row.probe : null;
}

function runOne({ row, field, value, scope, runProbe, envVar, env, candidates, route = "cli" }) {
  ...
  // the bug this parameter exists to prevent: a customer with no `gh` would have
  // had a `gh` command built for them and refused for the wrong reason.
  const template = route === "mcp" && row?.mcpProbe ? null : templateFor(row, field);
  ...
  if (template) {
    ...
    if (!filled.ok) {
      ...
      return {
        ...
        gap: {
          ...
          nextAction: nextActionFor(errorClass, { value, envVar }),
    ...
    const tool = interpolate(String(row.mcpProbe.tool ?? ""), merged, { leaders: [] });
    // The tool NAME is not a shell command, so interpolate's runnability check
    // does not apply — only placeholder resolution does.
    const toolName = tool.command;
    if (/\{[A-Za-z0-9_]+\}/.test(toolName)) {
      ...
      return {
        ...
        gap: {
          ...
          envVar: envVar ?? null,
          gapClass: classifyGap({ errorClass, env, row }),
          nextAction: `The MCP tool name for this capability is unresolved — answer it during setup, then re-run.`,
        },
      };
    }
    request = { kind: "mcp", tool: toolName, args: row.mcpProbe.args ?? {} };
  } else {
    const errorClass = "not-installed";
    return {
      field,
      value,
      ok: false,
  ...
  return {
    ...
    gap: {
      ...
      nextAction: nextActionFor(errorClass, { value, envVar, suggestion, tool: leaders[0] }),
    },
  };
}
...
export function verifyCapability({
  ...
  const results = targets.map((t) =>
    runOne({ row, field: t.field, value: t.value, scope, runProbe, envVar, env, candidates }),
  );
...
export function verifyGithub({
  ...
  const executables = row?.fingerprints?.executables ?? [];
  const mcpFingerprints = row?.fingerprints?.mcp ?? [];
  const have = (env.executables ?? []).map((e) => String(e).toLowerCase());
  const servers = (env.mcpServers ?? []).map((s) => String(s).toLowerCase());

  const cliRoute = executables.find((e) => have.includes(String(e).toLowerCase()));
  const mcpRoute = mcpFingerprints.find((f) => servers.some((s) => s.includes(String(f).toLowerCase())));

  if (!cliRoute && !mcpRoute) {
    return {
      verified: false,
      blocking: true,
      via: null,
  ...
  const targets = [];
  if (scope.repo !== undefined) targets.push({ field: "repo", value: scope.repo });
  if (scope.branch !== undefined) targets.push({ field: "baseBranch", value: scope.branch });

  const route = cliRoute ? "cli" : "mcp";
  ...
  const results = targets.map((t) => {
    ...
    if (route === "mcp" && t.field === "baseBranch") {
