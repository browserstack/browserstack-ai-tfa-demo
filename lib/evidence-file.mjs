...
function mergeSweeps(cur = [], incoming = []) {
  ...
  for (const s of [...(cur ?? []), ...(incoming ?? [])]) {
    const via = String(s?.via ?? "").trim();
    if (!via) continue; // a sweep that does not name its source proves nothing
    const prev = by.get(via);
    by.set(via, prev ? { ...prev, ...s, via, block: pickLeaf(prev.block, s.block) } : { ...s, via });
  }
  return [...by.values()];
}
