...
const DESTRUCTIVE_VERB =
  /(?:^|[\s/-])(rm|remove|delete|destroy|prune|purge|terminate|kill|stop|start|restart|reboot|drain|cordon|uninstall|scale|rollout|apply|replace|patch|put|post|create|update|dispatch|publish|push|upload|sync|migrate)(?:$|[\s=-])/i;
// The hyphen is in BOTH boundary classes on purpose: AWS-style verbs are
// hyphenated (`terminate-instances`, `delete-object`) and destructive flags are
// dashed (`rsync --delete`). It costs some false positives on names like
// `app=non-stop-worker`, which cost one refetch each.
...
const MUTATING = { test: (c) => MUTATING_TOOL.test(c) || DESTRUCTIVE_VERB.test(c) };
...
export function isRunnable(command) {
  ...
  const leader = leaderOf(segments[0]);
