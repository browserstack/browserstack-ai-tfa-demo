...
/** True if the segment carries a shell metacharacter OUTSIDE quotes.
 *
 ...
 * and the operator check sails straight past it. Not exploitable on its own (the
 ...
 *
 * Quoting is respected, mirroring hasUnquotedRedirect: a deliberately quoted
 * argument may contain anything; an unquoted metacharacter may not. No legitimate
 * repo, branch, namespace or index value needs one.
 */
function hasUnquotedMetachar(seg) {
  const META = new Set([";", "|", "&", "`", "$", "(", ")", "\n", "\r"]);
  let quote = null;
  for (const ch of String(seg ?? "")) {
    if (quote) { if (ch === quote) quote = null; continue; }
...
export function isProbeRunnable(command, { leaders = [] } = {}) {
  ...
  const meta = hasUnquotedMetachar(seg);
  if (meta) {
    return {
      ok: false,
  ...
  const op = argv.find((t) => OPERATOR_TOKENS.has(t));
  if (op) return { ok: false, reason: `'${op}' is a shell operator and is not allowed in a probe` };
  ...
  const verb = argv.slice(1).find((t) => DESTRUCTIVE_VERBS.has(t.toLowerCase()));
  if (verb) {
