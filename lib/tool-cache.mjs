...
// Writes must never be cached. The named-CLI alternations below stay because they
// encode real per-tool knowledge (`gh api -X POST`, `kubectl port-forward`), but they
// only ever covered gh, git, kubectl and curl — so `docker container prune -f`,
// `aws ec2 terminate-instances`, `nomad job stop`, `pm2 delete all`,
// `flyctl apps destroy`, `helm uninstall` and `terraform destroy` all read as
// cacheable. The same single-technology bias the capability table had, in the cache.
//
// DESTRUCTIVE_VERB is leader-agnostic and catches them by what they DO. A false
// positive here costs one refetch; a false negative caches the output of a
// state-changing command and hands it back as current. The asymmetry decides the
// direction.
const DESTRUCTIVE_VERBS = new Set([
  "rm", "remove", "delete", "destroy", "prune", "purge", "terminate", "kill", "stop",
  "start", "restart", "reboot", "drain", "cordon", "uncordon", "uninstall", "scale",
  "apply", "replace", "put", "post", "create", "update",
  "dispatch", "publish", "push", "upload", "sync", "migrate", "write",
  // NOT here: `rollout` and `patch`. `kubectl rollout status` is a read and
  // `git log --patch` is a diff format; the destructive forms of both
  // (`rollout undo|restart|pause|resume`, `kubectl patch`) are already covered
  // per-tool by MUTATING_TOOL, which knows the subcommand context this set
  // cannot. A generic list that ignores context is how a read gets refused.
]);

/** Words that state an action even in flag position — never a format selector. */
const DESTRUCTIVE_FLAGS = new Set(["delete", "destroy", "remove", "prune", "purge"]);

/**
 * Does any TOKEN of this command name a destructive verb?
 *
 * Tokens, not the raw string. Matching the string refused the wrapper's single
 * biggest read — `gh api repos/o/r/contents/src/update-user.js`, because the PATH
 * contains `update-`; also `git log --patch`, `git show abc:src/sync-service/…`
 * and `kubectl get pods -n payments-sync`. File contents were 126 of 407 forge
 * calls on one measured run, and `isRunnable` calls this FIRST, so `cached-exec`
 * exited 2 without running at all. "A false positive costs one refetch" was wrong
 * about its own code path.
 *
 * This is what the deleted isProbeRunnable did, and it was more precise than what
 * replaced it — worth recording, because the cruder version looked like a
 * generalisation and was a regression.
 *
 * Compared against the whole token and against each `=`-separated part, so
 * `--method=DELETE` is caught; a hyphenated word like `payments-sync` is not,
 * because it is not the verb `sync`. Bare flags without a value (`--patch`) are
 * deliberately not treated as verbs: as a flag it selects a diff format, and
 * refusing it would cost a real read. That leaves `rsync --delete` uncaught —
 * accepted knowingly, because this is a CACHEABILITY judgement over commands the
 * agent composed itself, not a safety boundary over untrusted input.
 */
function hasDestructiveToken(command) {
  let argv;
  try {
    argv = tokenize(command);
  } catch {
    return false; // unparseable: the redirect/quote checks below reject it anyway
  }
  const hit = (w) => DESTRUCTIVE_VERBS.has(String(w).replace(/^-+/, "").toLowerCase());

  for (const raw of argv.slice(1)) {
    if (raw.includes("/")) continue; // a path, not a verb

    if (raw.includes("=")) {
      // A flag with a value: `--method=DELETE`. EXACT match on each side only —
      // hyphen-splitting here would read `app=sync-worker` as the verb `sync`,
      // and a workload label is not a write.
      if (raw.split("=").some(hit)) return true;
      continue;
    }
    if (raw.startsWith("-")) {
      // A bare flag usually selects a FORMAT (`git log --patch`), which is why
      // flags are skipped. But a few words are never a format: `--delete`,
      // `--prune` and friends state an action whatever precedes them, so
      // `rsync --delete` is a write even though it names no subcommand.
      if (DESTRUCTIVE_FLAGS.has(raw.replace(/^-+/, "").toLowerCase())) return true;
      continue;
    }

    // A bare token, i.e. the subcommand position. Hyphen-split the FIRST segment
    // so AWS-style verb-noun subcommands are caught (`terminate-instances`,
    // `delete-object`) while a hyphenated NAME is not (`payments-sync` splits to
    // `payments`, which is not a verb).
    if (hit(raw) || hit(raw.split("-")[0])) return true;
  }
  return false;
}

const MUTATING_TOOL = /\b(rm|mv|cp|dd|truncate|tee)\b|\bgit\s+(push|commit|merge|rebase|reset|checkout|clean)\b|\bgh\s+(pr\s+(create|merge|close|edit|comment|review)|issue\s+(create|close|edit|comment)|release\s+create|repo\s+(create|delete)|api\s+(-X\s*)?(POST|PUT|PATCH|DELETE))|\bkubectl\s+(apply|delete|edit|patch|scale|create|replace|annotate|label|cordon|drain|exec|cp|port-forward|rollout\s+(undo|restart|pause|resume))\b|\bcurl\b[^|]*\s-(X|-request)\s*(POST|PUT|PATCH|DELETE)/i;

/**
 * Is this command a write?
 *
 * Applied to the FETCH SEGMENT only — never to the filter chain. A filter runs as
 * a pure text transform over the fetch's output and cannot mutate anything, so
 * `kubectl describe pod x | grep -i restart` was refused for the word "restart"
 * appearing in a grep pattern. That refusal is exit 2 with EMPTY stdout and the
 * reason on stderr, and this module's own documented idiom permits `2>/dev/null` —
 * so through a silenced pipe a refusal is indistinguishable from "no results"
 * from a wrapper the prose promises "behaves exactly like the raw command". A
 * false NEGATIVE in the evidence, not a lost cache hit.
 */
export function isCacheable(command) {
  const c = String(command ?? "");
  const fetch = splitPipeline(c).map(stripStderrPlumbing).filter((x) => x.length > 0)[0] ?? c;
  return !(MUTATING_TOOL.test(fetch) || hasDestructiveToken(fetch));
}
...
// What this wrapper will NOT run as the fetch, rather than what it will.
//
// This was an allowlist of four products — `gh|kubectl|curl|git` — which meant a
// team on any other stack got no caching at all: a `logcli`, `flyctl`,
// `newrelic-cli` or `aws` read was refused outright, so the whole benefit was
// reserved for teams using those four tools. The same single-technology bias as
// the capability table, one subsystem over.
//
// Inverted, the rule is generic AND states the actual safety property: a shell or
// interpreter as the fetch leader means the real command is inside an argument,
// where neither `isCacheable`'s write check nor the pipeline split can see it.
// Everything else is a data-fetching binary as far as this wrapper is concerned —
// it runs argv through execFileSync with no shell, and a write is refused
// separately by isCacheable.
const REFUSED_LEADERS = new Set([
  "bash", "sh", "zsh", "ksh", "dash", "fish", "csh", "tcsh",
  "python", "python2", "python3", "node", "deno", "bun", "ruby", "perl", "php", "lua",
  "osascript", "env", "eval", "exec", "source", "xargs", "find",
  "awk", "gawk", "sed", "nc", "ncat", "socat", "ssh", "telnet",
]);

/** The leader of a command segment, without path or arguments. */
function leaderOf(segment) {
...
export function isRunnable(command) {
  ...
  // The leader is checked on the TOKENIZED argv, which is what execFileSync
  // actually receives. Checking the raw segment let two quote characters through:
  // `"bash" -c "id"` read as leader `"bash"`, matched nothing in the set, and then
  // tokenize handed `["bash","-c","id"]` to the executor — the same
  // two-quoting-models-over-one-string divergence found in an earlier round, in a
  // different function.
  const leader = leaderOf(parsed[0]?.[0] ?? "");
  if (!leader) return { ok: false, reason: "empty command" };
  if (REFUSED_LEADERS.has(leader)) {
    return {
      ok: false,
      reason:
        `'${leader}' is a shell or interpreter, so the real command is inside an argument ` +
        `where the write check and the pipeline split cannot see it — run the fetch binary directly`,
    };
  }
...
/** Verbs that make a tool a write, or a read of work still in progress. */
const NEVER_CACHE_VERBS = new Set([
  "submit", "trigger", "create", "update", "delete", "dispatch", "write",
  "post", "put", "upload", "cancel", "approve", "start", "stop", "run",
]);

/**
 * Split an MCP tool name into words, across every convention it might use.
 *
 * Regex boundaries kept getting this wrong in both directions. `\b` never fires
 * beside an underscore, so `mcp__acme__submit_job` slipped through; and
 * `[^a-zA-Z]` cannot match the `R` in `createRcaTicket`, because camelCase's
 * boundary IS an uppercase letter. Splitting first and comparing words after is
 * the same fix the destructive-verb check needed — and it is legible, which the
 * regex was not.
 */
...
export function isCacheableMcp(toolName) {
  ...
  if (NEVER_CACHE_NAMES.some((p) => p.test(name))) return false;
  return !toolWords(name).some((w) => NEVER_CACHE_VERBS.has(w));
}
...
export function cachePut(cacheDir, key, entry, nowMs) {
  ...
  // OS temp dir and holds raw provider output — private repo source,
