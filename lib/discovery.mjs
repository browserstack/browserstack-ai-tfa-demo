...
export function planInterview({ table = {}, env = {}, assigned = {}, connectorSkills = [] } = {}) {
  ...
  for (const [capability, spec] of Object.entries(assigned)) {
    ...
    } else if (table[capability]?.resolvable === "always-asked") {
      violations.push({
        code: "assigned-always-asked",
        capability,
        message:
          `'${capability}' is always-asked, so an assignment cannot resolve it — it is the ` +
          `catch-all, and resolving it would swallow the unrecognised stack it exists to surface. ` +
          `Route the tool to the capability it SERVES, or leave it unassigned.`,
      });
  ...
  for (const [capability, row] of Object.entries(table)) {
    ...
    if (route && route.kind === "file") {
      ...
      // Also claim any present tool the via NAMES, so an assignment of
      // `via: "newrelic"` against an executable called `newrelic-cli` does not
      // report its own routed tool back as unassigned.
      for (const kind of ["executables", "mcpServers"]) {
        for (const present of env[kind] ?? []) {
          const p = norm(present);
          const v = norm(route.name);
          if (p.includes(v) || v.includes(p)) claimed.add(p);
        }
      }
      routes.push({
        capability,
