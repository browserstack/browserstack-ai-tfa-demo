...
export function startOfRunRefusal(readResult) {
  if (!readResult || readResult.ok !== true) {
    const code = readResult?.code ?? "no-context";
    if (code === "no-context") {
      return {
        refuse: true,
        code: "no-context",
        message: "No RCA setup context found, so there is nothing to run against.",
        nextAction: "Run the `rca-setup` skill once in this repo, then re-run this command.",
      };
    }
    ...
    return {
      ...
      nextAction:
        readResult?.message ??
        "Resolve the problem named above, or re-run the `rca-setup` skill to regenerate it.",
      path: readResult?.path ?? null,
    };
  }

  const githubOk = readResult.context?.verified?.github?.ok === true;
  if (!githubOk) {
    return {
      refuse: true,
      ...
      nextAction:
        "Re-run the `rca-setup` skill to verify GitHub (check the credential it names first).",
      path: readResult.path ?? null,
    };
  }
